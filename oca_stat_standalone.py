...
################################### OCA-STAT SCRAPER ########################
# This the the standalone version of the file that lives on the ec2 instance. 
# If there is an issue, this file could help to resolve or troubleshoot the
# problem. 
...
######## cleaned on a as-needed bases.`See utils.py` for function
    ...
    print("[OCA STAT] - Load to redshift")

    ########### load to redshift ######################
    # HIDE THE CRED STRING BEFORE GITHUB!!!!!
    cred_string="<see tabushextracts for cred string!!>"

    trunc_table = "TRUNCATE TABLE google.oca_stat; "
    copy_table = "COPY google.oca_stat FROM " 
    s3_schema = "'s3://bxdgoogletransformed/"
    path_table = "extract_{}/oca_stat.csv' credentials {} " \
    ...
    conn = None
    try:
        print("[OCA STAT] - open redshift connection {}" \
             .format(time.strftime("%Y-%m-%d %H:%M:%S")))
        ...
        cur = conn.cursor()
        print("[OCA STAT] - execute copy from s3 {}" \
             .format(time.strftime("%Y-%m-%d %H:%M:%S")))
        cur.execute(table_load)
        conn.commit()
        cur.close()
        print("[OCA STAT] - close redshift connection {}" \
             .format(time.strftime("%Y-%m-%d %H:%M:%S")))
